...
flashMultiBtn.onclick = async () => {
  try {
    ...
    const [bootloader, partitions, bootApp0, app, staticBin] = await Promise.all([
      loadBin("esp32s3_coder_kb.ino.bootloader.bin"),
      loadBin("esp32s3_coder_kb.ino.partitions.bin"),
      loadBin("esp32s3_coder_kb_boot_app0.bin"),
      loadBin("esp32s3_coder_kb.ino.bin"),
      loadBin("esp32s3_coder_kb_static_web.bin")
    ]);
    ...
    const fileArray = [
      ...
      { data: staticBin,  address: 0x290000 }
    ];
...
flashMergedBtn.onclick = async () => {
  try {
    let bin_list = {
      "esp32s3_coder_kb.ino.merged.bin": ""
    }
    //let git_assets = await getGitReleaseLink(bin_list);
    statusText.innerText = "Status: Downloading merged binary...";
    const mergedData = await loadBin("esp32s3_coder_kb.ino.merged.bin");
